...
public class FileUtil extends FileHelper {
     ...
     * @date 25. 9. 14.
     */
    public FileInfoVO chunkUpload(MultipartFile chunk, String uploadPath, String originFileName, int currentIndex, int totalChunkCnt) throws IOException, MessageException {
        ...
        log.info("저장한 청크파일 : {} size : {}", savedChunkFilePath, tempUpload.getSizeByte());
        ...
        if (fileCnt == totalChunkCnt) {
            // 모든 청크를 합쳐서 최종 파일 생성
            String savedFilePath = chunkUpload.save(uploadPath + "/" + chunkUpload.getUUIDFileName(originFileName), totalChunkCnt);
